import { CURRENT_MOCKS_SCHEMA_VERSION } from '../constants';
import {
  StoredDataV2,
  StoredDataV3,
  StoredDataV4,
...
/**
 * Migrates mock data to the current schema version
 * @param data - Raw data from storage
 * @returns Migrated data or undefined if migration fails
 */
export default function migrateMocks(data: any): StoredDataCurrent | undefined {
...
/**
 * Migrates mock data from version 2 to version 3
 * @param data - V2 mock data
 * @returns V3 mock data
 */
...
/**
 * Migrates mock data from version 3 to version 4
 * @param data - V3 mock data
 * @returns V4 mock data
 */
...
/**
 * Migrates mocks data structure from V3 to V4
 * @param data - V3 mocks data structure
 * @returns V4 mocks data structure
 */
...
/**
 * Migrates individual draft data from V3 to V4
 * @param data - V3 draft data
 * @returns V4 draft data
 */
export function migrateDraftV3toV4(data: StoredDraftDataV3): StoredDraftDataV4 {
  ...
  return {
    ...
    estimationSettings: {
      ...
      years: data.estimationSettings.years.map((year: number) => year.toString())
    }
  };
} 