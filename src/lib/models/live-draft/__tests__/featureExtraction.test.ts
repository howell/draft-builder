...
describe('FeatureExtractor', () => {
    ...
    describe('Feature extraction', () => {
        // Ranks are 0-indexed throughout (app convention)
        const mockPlayer = {
            defaultPosition: 'RB',
            positionRank: 4,
            overallRank: 14
        ...
        it('should extract basic player features correctly', () => {
            ...
            expect(features.playerPositionRank).toBe(4);
            expect(features.playerOverallRank).toBe(14);
        });

        it('should calculate position scarcity correctly', () => {
            // Create context with some RBs already drafted
            const context = createMockContext([
                { player: { defaultPosition: 'RB', positionRank: 0, overallRank: 2 }, price: 80 },
                { player: { defaultPosition: 'RB', positionRank: 1, overallRank: 7 }, price: 70 },
                { player: { defaultPosition: 'RB', positionRank: 3, overallRank: 11 }, price: 60 }
            ]);

            const features = extractor.extractFeatures(mockPlayer, context);

            // 3 higher-ranked RBs drafted, position rank 4 (0-indexed) = 3/(4+1) = 0.6
        ...
        it('should calculate overall scarcity correctly', () => {
            ...
            const context = createMockContext([
                { player: { defaultPosition: 'RB', positionRank: 0, overallRank: 0 }, price: 90 },
                { player: { defaultPosition: 'WR', positionRank: 1, overallRank: 4 }, price: 70 },
                { player: { defaultPosition: 'RB', positionRank: 2, overallRank: 9 }, price: 60 }
            ]);

            const features = extractor.extractFeatures(mockPlayer, context);

            // 3 higher overall ranked players drafted, overall rank 14 (0-indexed) = 3/(14+1) = 0.2
    ...
    describe('Scarcity metric calculation', () => {
        it('should handle zero higher-ranked players', () => {
            ...
            const player = { defaultPosition: 'QB', positionRank: 0, overallRank: 0 };
        ...
        it('should produce finite scarcity for the top-ranked player (rank 0)', () => {
            // Regression test: rank 0 used to divide by zero (0/0 = NaN), and a
            // NaN training row silently poisoned every regression weight.
            const picks = Array.from({ length: 5 }, (_, i) => ({
                player: { defaultPosition: 'QB', positionRank: i, overallRank: i },
                price: 50 - i,
                pickNumber: i + 1
            }));
            const context: DraftContext = {
            ...
            const topPlayer = { defaultPosition: 'QB', positionRank: 0, overallRank: 0 };
            const features = extractor.extractFeatures(topPlayer, context);

            expect(features.positionScarcity).toBe(0);
            expect(features.overallScarcity).toBe(0);
            expect(Object.values(features).filter(v => typeof v === 'number').every(Number.isFinite)).toBe(true);
        });
        ...
        it('should cap scarcity at 1.0', () => {
            const picks = Array.from({ length: 8 }, (_, i) => ({
                player: {
                    defaultPosition: 'QB',
                    positionRank: i, // Ranks 0, 1, 2, 3, 4, 5, 6, 7
                    overallRank: i
                },
                price: 50,
                pickNumber: i + 1
            ...
            // Player at 0-indexed rank 2: players 0, 1 are higher ranked = 2/(2+1) = 0.67
            const player = { defaultPosition: 'QB', positionRank: 2, overallRank: 2 };
            const features = extractor.extractFeatures(player, context);

            expect(features.positionScarcity).toBeCloseTo(0.67, 2);
            expect(features.overallScarcity).toBeCloseTo(0.67, 2);

            // Top-ranked player: no players rank higher, scarcity 0/(0+1) = 0
            const lowRankedPlayer = { defaultPosition: 'QB', positionRank: 0, overallRank: 0 };
    ...
    describe('Feature validation', () => {
        ...
        it('should reject features with invalid ranks', () => {
            const invalidFeatures = {
                playerPosition: 'RB',
                playerPositionRank: -1, // Invalid: ranks are 0-indexed, negative is impossible
                playerOverallRank: 15,
                positionScarcity: 0.3,
                overallScarcity: 0.2,
                budgetSpentPct: 0.25,
