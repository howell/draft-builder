...
import type { EstimationSettingsState, SearchSettingsState } from '@/types/storage';
...
export class DraftBuilderDB extends Dexie {
  ...
  async searchPlayers(draftId: number, query: string, position?: string): Promise<Player[]> {
    ...
    return collection.orderBy('overallRank').toArray();
  }
   ...
   * Set user setting
   */
  async setUserSetting(userId: string, type: string, key: string, data: any): Promise<void> {
...
export class DraftBuilderQueries {
  ...
  leagues(userId: string) {
    return {
      ...
      byPlatform: (platform: Platform) => 
        this.db.leagues.where(['userId', 'platform']).equals([userId, platform]).toArray(),
      favorites: () =>
        this.db.leagues.where(['userId', 'favorite']).equals([userId, true]).toArray()
    };
  }
  ...
  drafts(userId: string) {
    return {
      ...
      templates: () =>
        this.db.drafts.where(['userId', 'isTemplate']).equals([userId, true]).toArray(),
      recent: (limit: number = 10) =>
        this.db.drafts.where('userId').equals(userId)
        .orderBy('updatedAt').reverse().limit(limit).toArray()
    };
  }
  ...
  players(draftId: number) {
    return {
      ...
      byPosition: (position: string) =>
        this.db.players.where(['draftId', 'position']).equals([draftId, position]).toArray(),
